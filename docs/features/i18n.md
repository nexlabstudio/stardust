...
| Key | Default | Used In |
|-----|---------|---------|
| `search.placeholder` | `"Search documentation..."` | Search button |
...
Since Stardust currently reads `stardust.yaml` by default, swap the config file before each build:

```bash
# Build English
cp stardust.en.yaml stardust.yaml
stardust build

# Build Arabic
cp stardust.ar.yaml stardust.yaml
stardust build
